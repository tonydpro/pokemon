...
#define N_GROUPE	nombre_groupes
...
FILE* s_fopen(const char* cfile,const char* mode)
{
    ...
    {
		printf("Impossible d'ouvrir le fichier %s en mode %s.\n",cfile,mode);
		exit(EXIT_FAILURE);
	}
...
void afficher_console(SDL_Surface* image,const char* nom_font,int taille,char* console)
{
    ...
    for (i = 0 ; i < LARGEUR ; i++)
    {
        for (j = HAUTEUR - 40*AGRANDISSEMENT ; j < HAUTEUR ; j++)
            definirPixel(image,i,j,0x000000);
    }
    SDL_Rect postxt = {5*AGRANDISSEMENT,HAUTEUR - (30*AGRANDISSEMENT),LARGEUR - (5*AGRANDISSEMENT),20*AGRANDISSEMENT};
...
void afficher_xy(SDL_Surface* image,int x,int y)
{
    TTF_Font* font = s_TTF_OpenFont("pkmndp.ttf",16);
    char console[54];

	sprintf(console,"%d , %d",x,y);
